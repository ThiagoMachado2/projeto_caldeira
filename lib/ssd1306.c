...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
  ...
  if (c >= 'A' && c <= 'Z') {
      ...
      index = 63 * 8;           // Exclamação: posição após 'z'
  } else if (c == ':') {
      index = 64 * 8;           // Dois pontos: logo após a exclamação
  } else {
      index = 0; // Caracteres não reconhecidos serão um espaço em branco
  }
...
// Função auxiliar: desenha um caractere com escala
void ssd1306_draw_char_scaled(ssd1306_t *ssd, char c, uint8_t x, uint8_t y, uint8_t scale) {
  uint16_t index = 0;

  // Define o índice na tabela de fonte conforme o caractere
  if (c >= 'A' && c <= 'Z') {
      index = (c - 'A' + 11) * 8; // Letras maiúsculas
  } else if (c >= 'a' && c <= 'z') {
      index = (c - 'a' + 37) * 8; // Letras minúsculas
      ...
      index = 0; // Outros caracteres serão desenhados como espaço
  }

  // Para cada coluna (8 pixels de largura)
  for (uint8_t i = 0; i < 8; i++) {
      uint8_t line = font[index + i];
      // Para cada linha da coluna
      for (uint8_t j = 0; j < 8; j++) {
          // Verifica se o bit correspondente está aceso
          bool pixel_on = (line & (1 << j)) != 0;
          if (pixel_on) {
              // Desenha um bloco scale x scale
          ...
          // Se desejar limpar os pixels do fundo (quando pixel_off), 
          // você pode adicionar um bloco para desenhar "false"
          // else {
          //    for (uint8_t dx = 0; dx < scale; dx++) {
          //        for (uint8_t dy = 0; dy < scale; dy++) {
          //            ssd1306_pixel(ssd, x + i * scale + dx, y + j * scale + dy, false);
          //        }
          //    }
          // }
      }
...
// Função para desenhar uma string com escala
void ssd1306_draw_text(ssd1306_t *ssd, uint8_t x, uint8_t y, const char *text, uint8_t scale) {
  while (*text) {
      // Desenha o caractere atual com a escala definida
      ssd1306_draw_char_scaled(ssd, *text, x, y, scale);
      // Avança a posição x para o próximo caractere (largura de 8 pixels * scale)
      x += 8 * scale;
      text++;
      // Se ultrapassar a largura do display, pode-se implementar um wrap de linha (opcional)
      if (x + 8 * scale > ssd->width) {
          x = 0;
          y += 8 * scale;
          if (y + 8 * scale > ssd->height) {
              break; // Se não houver mais espaço, sai
          }
      }
  }
}